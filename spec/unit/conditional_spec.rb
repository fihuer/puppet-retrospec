...
describe "conditional" do
  ...
  describe 'one resource module' do
    ...
    it 'should initialize ' do
      ...
      conds = Conditional.find_conditionals(test_type.code)
      con = Conditional.new(conds.first)
  ...
  it 'should find nested resources' do
    my_path = File.expand_path(File.join('spec', 'fixtures', 'fixture_modules', 'one_resource_module'))
    m = Utilities::PuppetModule.send :new
    m.module_path = my_path
    m.create_tmp_module_path(my_path)
    test_type = m.types.find {|x| x.name == 'one_resource::another_resource'}
    conds = Conditional.find_conditionals(test_type.code)
    expect(conds.length).to eq(3)
    conds.each do |cond|
      require 'pry'
      binding.pry
      puts Conditional.new(cond).testcase
    end
  end
